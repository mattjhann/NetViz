...
import { DNS_STEPS } from '../data/dnsResolution.js';
...
export default function DnsResolutionPage() {
  ...
  const hero = (
    <header className="hero">
      ...
      <motion.p className="hero__lede" {...rise(0.24)}>
        ...
        www.example.com into an IP address. Step through a query as it walks the DNS
        hierarchy — root to TLD to authoritative — and hover any field to see what it carries.
      </motion.p>
    </header>
  );
  ...
  return (
    <ScrollDeck
      stepCount={DNS_STEPS.length}
      hero={hero}
      footer={footer}
      renderStage={(activeIndex, onSelectStep) => (
        <DnsStage activeIndex={activeIndex} onSelectStep={onSelectStep} />
      )}
    />
  );
}
